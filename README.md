...
Reveals plaintext passwords from their unsalted hash.
...
- *non-invertible*, or a *one-way* function: Given a hash value _h_, and plaintext _p_, there exists no mathematical function that satisfies $f(h) = p$. So, if only _h_ is known, there is not a simple method to reverse-engineer the original input _p_.
...
The CrackStation library, by abstraction of underlying complexity, provides that elusive "simple method" to reverse-engineer a hash function. Given an **unsalted** encrypted hash, it will reveal the original plaintext. Assuming determinism and strong collision resistance of the hash function, the plaintext found by the CrackStation is almost guaranteed to be the corresponding original plaintext that was hashed. As a consequence of hash function "*non-invertible* principle, the CrackStation's capacity to crack is limited to a finite set of plaintext possibilities.
...
CrackStation's strategy trades storage requirements for computation time. It relies on the storage of pre-computed hashes generated from all permutations of given string possibilities. These hashes are structured as a look-up table (a dictionary). When CrackStation is asked to crack a given hash, it searches for that hash in the table and, if it exists, returns the plaintext password, achieving a $O(1)$ run-time for look-ups. However, this is definitely not scalable. To guarantee the capacity to crack a password of even just 5 characters, the look-up table would be required to hold over a billion entries -- approximately 70 GB of storage-space!

The current strategy relies on a questionable assumption; namely, the plaintext passwords users might want to crack are completely random. Perhaps instead, it is more realistic to consider most passwords are generated by humans and subject to human biases. A strategy which relies on hashing combinations of dictionary words or common words specific to a given user's use case should be adopted. However, efforts to implement complex but storage-saving strategies may not be justifed or useful. The underlying cracking strategy is invalidated by the modern techniques of salting or iterative hashing that are used by most entities to store passwords securely.

## Mission Statement
CrackStation can be used to identify weak, easily crackable passwords and hopefully discourage you or your users from using them. CrackStation's ease and speed at cracking weak passwords can emphasize the importance of choosing [strong passwords](https://security.harvard.edu/use-strong-passwords) and justify complex password requirements for your users. Password requirements are not only just annoying barriers between you and your new account but are important for securing your data. From a cybersecurity and password storage perspective, CrackStation cautions about the dangers of neglecting to salt passwords before storing them.

Although CrackStation is just a tool that does not regulate the way it is used, the authors of the library ask that you follow the "[hacker ethic](http://hacks.mit.edu/Hacks/misc/ethics.html)" outlined by MIT. According to the hacker ethic, a hack must:
- be safe
- not damage anything
...
```
The `decrypt` function is the main way to utilize the CrackStation. 
Parameters:
